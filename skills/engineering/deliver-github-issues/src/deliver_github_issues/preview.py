...
def render_preview(queue: dict[str, Any], policy: dict[str, Any]) -> str:
    lines = [
    ...
    for item in queue["issues"]:
        ...
        lines.extend(
            [
                ...
                f"#{number}: generate metadata with {policy['metadataAgent']['provider']}; commit, push, create PR; wait for {required}",
